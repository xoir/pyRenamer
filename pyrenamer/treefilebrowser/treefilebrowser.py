...
class TreeFileBrowser(GObject.GObject):
    ...
    __gproperties__ = {
        ...
        "rules-hint": (
            GObject.TYPE_BOOLEAN,
            "rules hint",
            "show rows background in alternate colors",
            True,
            GObject.PARAM_READWRITE,
        ),
        "root": (
            GObject.TYPE_STRING,
            "initial path",
            "initial path selected on tree browser",
            "/",
            GObject.PARAM_READWRITE,
        ),
    }
    ...
    __gsignals__ = {
        "row-expanded": (GObject.SignalFlags.RUN_LAST, None, (GObject.TYPE_STRING,)),
    ...
    def __init__(self, root=None):
        ...
        self.root = "/"
        if root != None and ospath.isdir(root):
            self.root = root
    ...
    def set_show_hidden(self, hidden):
        ...
        if activedir != None:
            if "/." in activedir:
                self.set_active_dir(self.root)
            else:
                self.set_active_dir(activedir)
    ...
    def set_active_dir(self, directory):
        ...
        if len(directory) > 1 and directory[-1] != "/":
            directory += "/"
        if len(rootdir) > 1 and rootdir[-1] != "/":
            rootdir += "/"
        ...
        if directory == rootdir:
            ...
            if len(dirtree) > 1:
                dirtree[0] = "/"
                for i in range(len(dirtree) - 1):
                    dirtree[i + 1] = dirtree[i + 1] + "/"
            ...
            if len(roottree) > 1:
                roottree[0] = "/"
                for i in range(len(roottree) - 1):
                    roottree[i + 1] = roottree[i + 1] + "/"

            # Check if the dir is in the same path as the desired root
            long = len(roottree)
            for i in range(long):
                if roottree[i] != dirtree[i]:
                    return False
                ...
                if i[0] != "." or (self.show_hidden and i[0] == "."):
                    ...
                    if ospath.isdir(path):
                        ...
                        if subdir != []:
                            for i in subdir:
                                if (
                                    ospath.isdir(ospath.join(path, i))
                                    or not self.show_only_dirs
                                ):
                                    if i[0] != "." or (
                                        self.show_hidden and i[0] == "."
                                    ):
